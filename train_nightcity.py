...
#     n, c, h, w = bd_pre.size()
#
#     # Choose a channel (assuming the first channel here)
#     selected_channel = 0
#     selected_bd_pre = bd_pre[:, selected_channel, :, :].unsqueeze(1)
#     # Reshape the dimensions
#     log_p = selected_bd_pre.view(1, h, w, 1)
#     target_t = target.view(-1, 1).float()  # Convert target to float
#     # print("log_p shape:", log_p.shape)
#     # print("target_t shape:", target_t.shape)
#     pos_index = (target_t == 1).view(-1, 1)
#     neg_index = (target_t == 0).view(-1, 1)
#     # print("pos_index shape:", pos_index.shape)
#     # print("neg_index shape:", neg_index.shape)
#     pos_num = pos_index.sum()
#     neg_num = neg_index.sum()
#     sum_num = pos_num + neg_num
#
#     if sum_num != 0:
#         weight = torch.where(pos_index, neg_num * 1.0 / sum_num, pos_num * 1.0 / sum_num)
#
#         # Save the original log_p shape
#         original_log_p_shape = log_p.shape
#
#         # Ensure that log_p and target_t have the same size
#         log_p = log_p.reshape(-1, 1)
#         target_t = target_t.reshape(-1, 1)
#
#         # Use the saved shape for binary cross-entropy loss calculation
#         loss = F.binary_cross_entropy_with_logits(log_p, target_t, weight, reduction='mean')
#
#         # Restore the original log_p shape
#         log_p = log_p.view(*original_log_p_shape)
#
#     else:
#         # Handle the case where sum_num is zero to avoid division by zero
#         loss = torch.tensor(0.0, dtype=log_p.dtype)
#
#     return loss
#
#
...
def main():
 ...
 #           loss_boundary= boundary_loss(pred_c, labels)
...
#            loss_boundary_value+=loss_boundary.item()/args.iter_size
 ...
 #           loss_boundary_night=boundary_loss(pred_c_nightcity, labels)
  ...
  #          loss_boundary_value_night += loss_boundary_night.item() / args.iter_size



        optimizer.step()
        if i_iter % 1 == 0:# and i_iter != 0:
            print(
            ...
            # print(
            #     'iter = {0:8d}/{1:8d}, loss_seg = {2:.3f}, loss_seg_value_nightcity = {3:.3f},loss_boundary = {4:.3f},loss_boundary_night = {5:.3f}'.format(
            #         i_iter, args.num_steps, loss_seg_value, loss_seg_value_nightcity, loss_boundary_value,
            #         loss_boundary_value_night))
